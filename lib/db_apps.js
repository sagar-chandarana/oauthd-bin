...
exports.create = check({
  ...
  err.check('tokenExpiry', data.tokenExpiry, 'number');
  if (data.domains) {
    _ref = data.domains;
  ...
  return db.redis.incr('a:i', function(err, idapp) {
    ...
    prefix = 'a:' + idapp + ':';
    cmds = [['mset', prefix + 'name', data.name, prefix + 'key', key, prefix + 'secret', secret, prefix + 'tokenExpiry', data.tokenExpiry], ['hset', 'a:keys', key, idapp]];
    if (data.domains) {
      _ref1 = data.domains;
...
exports.getById = check('int', function(idapp, callback) {
  var prefix;
  prefix = 'a:' + idapp + ':';
  return db.redis.mget([prefix + 'name', prefix + 'key', prefix + 'secret', prefix + 'tokenExpiry'], function(err, replies) {
    if (err) {
      return callback(err);
    }
    return callback(null, {
      ...
      key: replies[1],
      secret: replies[2],
      tokenExpiry: parseInt(replies[3])
    });
  });
});
  ...
  return db.redis.hget('a:keys', key, function(err, idapp) {
    ...
    return async.parallel([
      function(callback) {
        ...
        upinfos = [];
        if (data.tokenExpiry) {
          upinfos.push('a:' + idapp + ':tokenExpiry');
          upinfos.push(data.tokenExpiry);
        }
        if (!upinfos.length) {
          return callback();
        }
